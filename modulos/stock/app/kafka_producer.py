        ...
        for attempt in range(max_retries):
            try:
                _producer_instance = KafkaProducer(
                    ...
                    client_id='stock-service-producer',
                )
                print("INFO: KafkaProducer conectado exitosamente.")
                break
            except NoBrokersAvailable as e:
                print(
                    f"WARN: Intento {attempt + 1}/{max_retries} fallido al conectar con Kafka (NoBrokersAvailable): {e}")
...
def enviar_evento_stock(accion: str, datos: dict, usuario_id: str = None):
    """Función específica para eventos de stock"""
    evento = {
        "accion": accion,
        "modulo": "STOCK",
        "datos": datos,
    ...
    enviar_evento("stock-events", evento)


def enviar_alerta_stock(tipo_alerta: str, producto_info: dict, detalle: str = None):
    ...
    alerta = {
        ...
        "detalle": detalle,
        "timestamp": time.time(),
        "prioridad": "ALTA" if tipo_alerta in ["STOCK_CRITICO", "PRODUCTO_VENCIDO"] else "MEDIA"
    }
    enviar_evento("stock-alerts", alerta)