...
# Cargar variables de entorno desde .env ubicado en la raíz del microservicio 'ventas'
...
def require_role(required_roles: list):
    """
    Decorator para verificar roles específicos en el token.
    Args:
        required_roles: Lista de roles permitidos ['admin', 'vendedor', 'cajero']
    """

    def _require_role(payload: dict):
        user_roles = payload.get("https://mi-api/roles", [])
        if not any(role in user_roles for role in required_roles):
            raise HTTPException(
                status_code=403,
                detail=f"Acceso denegado. Se requiere uno de los siguientes roles: {required_roles}"
            )
        return payload
