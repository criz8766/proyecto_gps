...
# Cargar variables de entorno desde .env ubicado en la raíz del microservicio 'usuarios'
...
def require_permissions(required_permissions: List[str]):
    """
    Decorator para verificar permisos específicos en las rutas.
    """

    def permission_checker(current_user_payload: dict = Depends(validate_token)):
...
def require_admin(current_user_payload: dict = Depends(validate_token)):
    """
    Verificar si el usuario tiene permisos de administrador.
    """
    user_permissions = current_user_payload.get("permissions", [])

    if "gestionar:usuarios" not in user_permissions:
        raise HTTPException(
            status_code=403,
