...
# Cargar variables de entorno desde .env ubicado en la raíz del microservicio 'caja'
...
def require_permissions(required_permissions: list):
    """
    Decorador para validar permisos específicos del usuario
    """

    async def validate_permissions(current_user_payload: dict = Security(validate_token)):
        user_permissions = current_user_payload.get("permissions", [])

        # Verificar si el usuario tiene al menos uno de los permisos requeridos
        if not any(perm in user_permissions for perm in required_permissions):
            available_perms = ", ".join(user_permissions) if user_permissions else "ninguno"
            required_perms = ", ".join(required_permissions)
            print(f"Acceso denegado. Permisos requeridos: {required_perms}. Permisos del usuario: {available_perms}")
            raise HTTPException(
                status_code=403,
                detail=f"Acceso denegado. Se requiere uno de estos permisos: {required_perms}"
            )
